...
let DATA = JSON.parse(str) || []; // присваеваем константе, елси он существует
render();

const inp = document.querySelector("#value");
const btn = document.querySelector("#add");
const btnClear = document.querySelector("#removeAll");
...
//     event.target.parentNode.classList.toggle("block_done");
//   }
// });

// функция записывает выполненые задачи в хранилище
tasks.addEventListener("click", (event) => {
...
btnDone.addEventListener("click", () => {
  ...
  for (i = 0; i < inputs.length; i++) {
    if (!inputs[i].checked) {
      ...
      DATA.unshift({ title: elem, state: 0 });
      // console.log(elem);
    }
  }
  // console.log(DATA);
